...
/**
 * 联系图标组件 (contactIcons.vue)
 *
 * 耦合关系：
 *   - stores/site.ts → 读取 contactOrder / contacts / getHoveredContact
 *
 * 应用方式：
 *   <contactIcons :compact="false" title="Contacts" />
 *
 * 函数表：
 *   - handleEnter(key): 鼠标进入图标，设置悬停状态
 *   - handleLeave():    鼠标离开图标，清除悬停状态
 */
...
const { setHoveredContact } = siteStore;
